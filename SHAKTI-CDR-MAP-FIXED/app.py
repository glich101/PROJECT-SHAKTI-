...
def map_data(data_type, df):
    ...
    """
    app.logger.info(f"MAP_DATA: Request for {data_type}, dataframe shape: {df.shape}")
    app.logger.info(f"MAP_DATA: DataFrame columns: {df.columns.tolist()}")
    app.logger.info(f"MAP_DATA: Query parameters: {dict(request.args)}")
    try:
        ...
        if not lat_col or not lon_col: # Attempt auto-detection if not fully specified
            ...
            lat_keywords = ['latitude', 'lat', 'ycoord', 'start_lat', 'end_lat', 'cell_lat', 'lat_start', 'lat_end', 'latitude_start', 'latitude_end', 'y', 'coord_y']
            lon_keywords = ['longitude', 'lon', 'long', 'xcoord', 'start_lon', 'end_lon', 'cell_lon', 'lon_start', 'lon_end', 'longitude_start', 'longitude_end', 'x', 'coord_x']

            app.logger.info(f"MAP_DATA: Auto-detecting lat/lon columns from: {df.columns.tolist()}")
            ...
            for c in df.columns:
                ...
                if not found_lat:
                    for keyword in lat_keywords:
                        if keyword in cl:
                            # Additional check: ensure this column contains numeric data
                            try:
                                sample_values = df[c].head(5).dropna()
                                if len(sample_values) > 0 and pd.api.types.is_numeric_dtype(sample_values):
                                    found_lat = c
                                    app.logger.info(f"MAP_DATA: Found lat column: {c} (numeric check passed)")
                                    break
                                else:
                                    app.logger.debug(f"MAP_DATA: Skipping {c} - not numeric")
                            except Exception as e:
                                app.logger.debug(f"MAP_DATA: Error checking {c}: {e}")
                if not found_lon:
                    for keyword in lon_keywords:
                        if keyword in cl:
                            ...
                            try:
                                ...
                                if len(sample_values) > 0 and pd.api.types.is_numeric_dtype(sample_values):
                                    found_lon = c
                                    app.logger.info(f"MAP_DATA: Found lon column: {c} (numeric check passed)")
                                    break
                                else:
                                    app.logger.debug(f"MAP_DATA: Skipping {c} - not numeric")
                            except Exception as e:
            ...
            if found_lat and found_lon and found_lat != found_lon:
                ...
                app.logger.info(f"MAP_DATA: Auto-detected lat_col='{lat_col}', lon_col='{lon_col}'")
            else:
                app.logger.warning(f"MAP_DATA: Auto-detection failed: found_lat={found_lat}, found_lon={found_lon}")
        ...
        for idx, row in df_coords.head(5000).iterrows():
            ...
            # Get the full row from the original dataframe using the index
            try:
                full_row = df.loc[idx]
                ...
                app.logger.debug(f"MAP_DATA: Could not get full row data for index {idx}: {e}")
        ...
        app.logger.info(f"MAP_DATA: Returning {len(points)} points with center at {center_lat}, {center_lon}")
        return jsonify({'points': points, 'total': total, 'center': {'lat': center_lat, 'lon': center_lon}})
    except Exception as e:
        app.logger.error(f"MAP_DATA: Failed to generate map data for {data_type} with lat_col='{lat_col}', lon_col='{lon_col}'. Error: {e}", exc_info=True)
